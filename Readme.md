...
Lets dive in into the world of **plugin based architectures** using **Web Components** and learn how to plug them into **React, Vue and Angular** apps dynamically by importing them in our app from outside the code base
...
This approach was used in the [Web components introduction article](https://academind.com/tutorials/web-components-introduction/) where a web component who lived inside `search-result.js` file,  and was imported inside an React, Vue ad Angular component with the following code:
...
2) **Importing the Web Component inside `index.html` file**, resulting in the web component not being bundled and being outside the code base. **This is the plugin based architecture approach!**

The way of importing it is:
...
**1) Maintainability**: adding the Web Component outside the source code means that the component is NOT bundled by Webpack (or any other tool the bundles code in your app) and doesn't become part of the final JavaScript bundles.
...
**2) Have a dedicated team of developers** that can work on the Web Component independently of the team working on the code base, because the app is split into two separate units: the code base and the web components.

**3) Fewer updates** on enterprise apps:  if changes are made to the Web Component, the file containing it is **only** delivered to the clients interested in the feature it implements, and not to **all** clients. 

As a note, enterprise apps are software products delivered to companies, and the apps might not be the same for all clients. On the other hand, apps like [academind.com](https://academind.com/) are considered normal apps, not enterprise ones, because the same app is delivered to everyone who enters to https://academind.com/. 

Another difference, is that enterprise apps are usually installed in private networks, not exposed to the general public like you and me, and are designed for internal usage.
...
We're gonna build the same app we built together in [Web components introduction article](https://academind.com/tutorials/web-components-introduction/), but this time, importing the web component following the plugin based architecture approach; this means: importing the web component in the `index.html` file of our app, and hence, not bundling it.

#### App functionality:

Image that we have a simple app that greets the name we enter in the input form:

![](./images/basic-app.jpg)



Now, we want to add a new feature, which is a link that opens the result of a Google search for the entered name:
...
The new feature can be implemented with a Web Component created with Vanilla JS. Let's create a file `search-result.js`:
...
In the following sections, we're gonna 
